...
from emg2qwerty.modules import (
    MultiBandRotationInvariantMLP, SpectrogramNorm, TDSConvEncoder,
    BiLSTMEncoder, CNNLSTMEncoder, BiRNNEncoder, CNNRNNEncoder, BiGRUEncoder, CNNGRUEncoder
)
...
class CNNLSTMCTCModule(pl.LightningModule):
    ...
    def __init__(
        ...
        # Feature extractor: normalize + MLP + flatten
        self.feature_extractor = nn.Sequential(
            SpectrogramNorm(channels=self.NUM_BANDS * self.ELECTRODE_CHANNELS),
            MultiBandRotationInvariantMLP(
                in_features=in_features,
...
class RNNCTCModule(pl.LightningModule):
    ...
    def __init__(
        ...
        # Classifier
        self.classifier = nn.Sequential(
            nn.Linear(rnn_hidden * 2, charset().num_classes),
            nn.LogSoftmax(dim=-1),
        )
        ...
        metrics = MetricCollection([CharacterErrorRates()])
        self.metrics = nn.ModuleDict({
            f"{p}_metrics": metrics.clone(prefix=f"{p}/")
    ...
    def _step(
        ...
        reg_loss = self.encoder.reg_loss
...
class GRUCTCModule(pl.LightningModule):
    ...
    def __init__(
        ...
        self.classifier = nn.Sequential(
            nn.Linear(gru_hidden * 2, charset().num_classes),
            nn.LogSoftmax(dim=-1),
        )
...
class CNNGRUCTCModule(pl.LightningModule):
    ...
    def __init__(
        ...
        self.classifier = nn.Sequential(
            ...
            nn.GELU(),
            nn.Dropout(0.3),
            nn.Linear(gru_hidden, charset().num_classes),
            nn.LogSoftmax(dim=-1),
        )
